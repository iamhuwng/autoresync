...
test('maps every Listening type to accepted source-assisted support', () => {
  ...
  for (const row of listening) {
    assert.equal(row.support.status, 'supported-through-source-assisted');
...
test('fails closed for unclassified, duplicated, falsely structural, stale, and unregistered types', () => {
  ...
  assert.match(errors, /taxonomy profile mapping drift/);
});
