...
const requireExisting = (target) => existsSync(target) ? readFileSync(target, 'utf8') : '';
...
const createFixture = () => {
  ...
  for (const item of sourceContract.codeEvidence) {
    const target = path.join(root, item.path);
    write(root, item.path, `${requireExisting(target)}// ${item.symbol}\n`);
  }
...
test('fails closed for duplicate ownership, missing symbol, and stale baseline successors', () => {
  ...
  try {
    ...
    fixture.contract.courseAuthorityAdapter.facts.find((fact) => fact.id === 'publication').path = 'book_delivery/publications/{bookId}/{publicationId}'; writeFileSync(fixture.contractPath, JSON.stringify(fixture.contract));
    assert.ok(inspectReadiness({ repo: fixture.root, contractPath: fixture.contractPath }).diagnostics.includes('publication_authority_incomplete'));
    fixture.contract.courseAuthorityAdapter = structuredClone(sourceContract.courseAuthorityAdapter); writeFileSync(fixture.contractPath, JSON.stringify(fixture.contract));
