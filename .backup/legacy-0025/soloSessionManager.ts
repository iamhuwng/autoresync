...
export async function createSoloSession(
    ...
    try {
        ...
        // The soloEnabled gate applies to self-study / public-library access only.
        // For course_material, the teacher's decision to add the test to a course module
        // IS the access grant — skip the solo mode check entirely.
        if (context.type !== 'course_material' && !material.soloConfig?.soloEnabled) {
            throw new Error('Solo mode is not enabled for this material');
        }

        // Get configuration from material — soloConfig may be absent for course_material context
        const soloConfig = material.soloConfig;
        const appliedConfig = {
            timerMinutes: soloConfig?.defaults?.timerMinutes ?? null,
            feedbackTiming: soloConfig?.defaults?.feedbackTiming ?? ('after_completion' as const),
        ...
        const session: SoloSession = {
            ...
            startedAt: Date.now(),
            timeSpent: 0,
            timeRemaining: appliedConfig.timerMinutes ? appliedConfig.timerMinutes * 60 : null,
            currentQuestion: 0,
...
export async function updateSoloSession(
    ...
    // Sanitize: Firebase RTDB rejects undefined values in update().
    // Strip any keys whose value is undefined before writing.
    const sanitized: Record<string, any> = {};
    for (const [k, v] of Object.entries(updates)) {
        if (v !== undefined) sanitized[k] = v;
    }
    if (Object.keys(sanitized).length === 0) return; // nothing to write

    try {
        const sessionRef = ref(database, `solo_sessions/${sessionId}`);
        await update(sessionRef, sanitized);
