...
- [ ] 2.0 Ship urgent severable upload-worker security hardening gate S0
  ...
  - [ ] 2.11 Human-assisted deploy gate: deploy only with explicit approval through the exact mechanism captured in 2.2. Pre-deploy prerequisite Packet 2K records the approved SQLite Durable Object replay ledger decision and local-only `UPLOAD_GRANT_REPLAY_LEDGER` implementation. Packet 2L records Option A local adapter/canary readiness only: thin browser facade, authenticated grant client, `VITE_R2_UPLOAD_WORKER_URL`, and dry-run-only `r2-upload-signer-s0-canary` config. Packet 2M records approved Phase A canary provisioning only: `r2-upload-signer-s0-canary` deployed to workers.dev with required `UPLOAD_GRANT_SECRET`, `UPLOAD_GRANT_REPLAY_LEDGER`, `UPLOAD_RATE_LIMITER`, `R2_BUCKET`, `PUBLIC_URL`, and `FIREBASE_PROJECT_ID` verification; production `r2-upload-signer` remained on version `20dd8429-5be1-4105-baed-f6dc5af68098`. Packet 2O Phase C readiness audit is BLOCKED: production remote bindings still lack `UPLOAD_GRANT_SECRET`, `UPLOAD_GRANT_REPLAY_LEDGER`, `UPLOAD_RATE_LIMITER`, `FIREBASE_PROJECT_ID`, and migration tag; production `cloudflare/wrangler.jsonc` still has invalid semantic namespace `prd0056-upload-worker-s0`; live Hosting still serves version `2ca9c185ac62dd7b` without the Task 2.11 authenticated grant client; canary remains on version `627f7503-8324-45d1-8e23-cdd02828111c` with required bindings. Production browser/Worker deploy, production traffic change, production config prep, secret mutation, rollback execution, R2 object mutation, deployed production authorized upload/move proof, version-pin proof, push, and final S0 acceptance remain unstarted. Verify configured bindings/secrets by name, deployed negative tests, and one authorized upload/move path without exposing token values, signed URLs, or raw keys in logs. Any Playwright/browser proof uses `--reporter=json > report.json`.
