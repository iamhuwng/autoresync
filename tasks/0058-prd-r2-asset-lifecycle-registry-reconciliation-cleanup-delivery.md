...
2026-07-02 abandoned-upload cleanup packet status: source now includes a trusted Worker cancel route for Listening upload sessions plus authoring UI wiring for replacement, section remove, discard, and upload abort. The browser sends only `uploadSessionId` plus optional `assetId`; the Worker derives and validates canonical temp keys, writes `cleanup-queued` before reference scan or trusted R2 deletion, checks retained references, deletes only unreferenced `temp/listening/` objects through trusted R2 binding, retries queued cleanup, and moves the upload session out of silent `active` state. Client cleanup calls retry before emitting diagnostics. Focused local tests, app build, Worker focused tests, Worker dry-run, production Worker deployment, and trusted cleanup execution passed. Owner approval was limited to deploying the cleanup route and deleting the nine exact `temp/listening/glMHCrzMnyS6AqFcb9I0nlOqQ6X2/...` candidates from the abandoned workflow. Post-cleanup proof showed R2 HEAD 404 for all nine candidate keys, nine RTDB sessions at `status:"abandoned"` with `abandonmentReason:"builder-cancel"`, zero `/media_assets/{assetId}` records, zero durable-root asset/key hits across `listening_authoring/drafts`, `listening_authoring/versions`, `tests`, `results`, `assignments`, and `sessions`, and zero permanent prefixes touched. Follow-up permanent setup deployed Worker version `dca3e056-142f-4cb4-9194-3117675f8889` with cron `0 * * * *`, future-only cutoff `LISTENING_UPLOAD_SESSION_SWEEP_NOT_BEFORE_MS=1782976636347`, scheduled trusted cleanup for expired future Listening temp upload sessions, aggregate sweep/metric writes without raw temp keys, deployed RTDB metric-operation rules, and provisioned enabled R2 lifecycle rule `expire-temp-prefix-after-one-day` for prefix `temp/`. Merge, push, first observed scheduled sweep run, durable cleanup execution, broader remote-state mutation, and historical orphan deletion remain explicitly unapproved/not performed in this packet.

Current deployed temp-upload cleanup authority is summarized in `documentation/architecture/listening-temp-upload-cleanup-authority.md`. That architecture note is the first read for future changes to Listening abandoned temp cleanup, cron cleanup, or `temp/` lifecycle fallback.
