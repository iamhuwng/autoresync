...
84a. Reading V2 submission/result generation requires a trusted HTTP backend boundary. The current implementation uses the deployed Cloudflare Worker route `POST /api/reading-v2/submit`. Firebase Functions are legacy/optional wrapper code only, not the first-release requirement. Any future replacement must verify Firebase Auth, load canonical snapshot/review projection data server-side, score without exposing answer keys to the browser, and write canonical result/index records server-side.
