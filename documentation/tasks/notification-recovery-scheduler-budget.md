...
The release check must confirm the account has two free Cron slots, count subrequests/CPU for a composed 30-recipient event and failed batch, and read back the scheduled queue progress. The first Linux emulator run also found an RTDB rules compiler error; the source fix needs a passing rerun before rules can be deployed.
