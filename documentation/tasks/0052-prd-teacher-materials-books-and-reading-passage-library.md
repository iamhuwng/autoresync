...
The feature must also reshape Teacher Materials discovery around admin-configurable test types such as IELTS, TOEIC, TOEFL, THCS, THPT, CELF/CEFR, and later institution-specific types.
...
15. Reading Passage source metadata must preserve original passage number 1/2/3 when available.
...
- The user said `TOFEL`; likely intended `TOEFL`.
- The user said `CELF`; likely intended `CEFR`, unless `CELF` is a deliberate local category.
...
An ordered tree/list item inside a Book.
...
Only content-bearing nodes can hold material references. Placeholder nodes are stored now for future page editing.
...
- As a teacher, I can see source metadata such as original full test and original passage number.
...
- As a teacher, I can attach existing materials to content-bearing nodes.
...
| FR-TAB-2 | `My Content` must show teacher-owned non-draft materials except Book-only and Reading-Passage-only views unless product decides duplication across tabs is allowed. |
...
- Should Reading Passages also appear in `My Content`, or only in `Reading Passage`? This affects discoverability and duplicate rows.
...
| FR-ADMIN-TT-9 | Only authorized admins can create/edit/deactivate Test Types. |
...
| FR-RP-4 | Reading Passage list row must show title, Test Type, question count, duration/time guidance if present, updated date, visibility, and source passage number. |
| FR-RP-5 | Source passage number must support at least `1`, `2`, `3`, and `unknown`. |
...
| FR-BOOK-3 | Book must have metadata: title, Test Type, tags, description, owner, visibility, createdAt, updatedAt. |
| FR-BOOK-4 | Book must store ordered nodes. |
| FR-BOOK-5 | Book must store material references under content-bearing nodes. |
...
| FR-BOOK-MODAL-4 | Modal minimum fields: title, Test Type, description, tags, visibility. |
| FR-BOOK-MODAL-5 | Test Type dropdown must use admin-configured active teacher-selectable Test Types. |
| FR-BOOK-MODAL-6 | Modal must create Book metadata plus default empty structure. |
| FR-BOOK-MODAL-7 | Modal must not create assignments, full tests, or Reading Passage copies. |
| FR-BOOK-MODAL-8 | After creation, teacher lands in Book editor/detail or remains on list based on final UX decision. |

Open decision:

- After creating Book, should teacher go directly to Book editor, or return to Book list with the new row selected?
...
| FR-NODE-6 | Content-bearing nodes can contain material references. |
| FR-NODE-7 | Node order must be stable and deterministic. |
| FR-NODE-8 | Node nesting must have a maximum depth defined before implementation. |
...
- Should Book support nested `chapter -> section -> test`, or only one flat ordered outline in V1?
...
| FR-REF-4 | Book can reference draft materials only if product explicitly allows draft refs. |
...
- Should Book refs bind to current live material or to the version at attach time?

Recommendation:

- Bind display ref to material id, but assignment actions must bind to an explicit published snapshot/version at assignment time.
...
- Open/Edit/View
- Assign Homework
- Duplicate/Use for Test (later or behind feature)
...
- `Reading Passage` active: CTA behavior must be decided. Options:
  1. `Create New Test` remains, because Reading Passage is mostly derived from Reading V2 publish/import.
  2. `Create Reading Passage` opens a passage import/create flow.
  3. Hide create CTA until direct passage authoring is designed.

Open decision:

- What should CTA do on `Reading Passage` tab?
...
- create/revise own Reading Passage materials
...
| Admin renames `TOFEL` to `TOEFL` | Store canonical key plus aliases; do not rewrite historical material blindly |
...
| Edge Case | Prevention |
|-----------|------------|
| Source passage number unknown | Display `Passage unknown`; do not invent 1/2/3 |
...
| Material appears multiple times in one Book | Allow or block based on final decision; must be explicit |
...
- Define which node types can contain children.
- Define which node types can contain material refs.
...
Resolve all open decisions in this PRD.
...
- publish/projection path for one-passage materials
...
- Each Reading Passage row shows source passage number if known.
...
- Teacher can create Book metadata.
- Book stores intro/TOC placeholders as empty nodes.
- Teacher can add section/chapter/test nodes.
- Teacher can attach existing material refs to content-bearing nodes.
...
- create Reading Passage entities only for new Reading V2 publishes
- create Reading Passage entities on explicit teacher action
...
1. Should Reading Passages also appear in `My Content`, or only in `Reading Passage`?
2. Should the 4-block Test Type module include an `All` block?
3. How should teachers clear an active Test Type filter if the 4-block module has only concrete Test Types?
4. What should the CTA do on the `Reading Passage` tab?
5. After creating a Book, should teacher land in Book editor or return to Book list?
6. Should Book support nested hierarchy in V1, or flat outline only?
7. Which node types can contain child nodes?
8. Which node types can contain material refs?
9. Should Book allow the same material to appear multiple times?
10. Should Book refs bind to live material, attach-time version, or assignment-time version?
11. Can a Book reference draft materials, or published materials only?
12. What visibility states should Book support in V1: private only, shared with own students, or public library eligible?
13. What visibility states should Reading Passage support in V1?
14. Should Reading Passage direct creation exist, or only extraction/import from Reading V2 full tests?
15. How should non-IELTS Reading Passage source order display when source has more/fewer than 3 passages?
16. Should teachers be able to bulk assign multiple Reading Passages at once in V1?
17. Should teachers be able to create a full Reading test from selected Reading Passages in V1 or later?
18. What exact label should be used: `CELF`, `CEFR`, or both?
19. Should `TOFEL` be treated as alias for `TOEFL`?
20. What role can manage Test Types: super admin only, or admin/teacher-admin too?
...
4. Build Reading Passage library and projection.
5. Build Reading Passage direct homework.
6. Build Book metadata/list/modal.
7. Build Book editor with placeholder nodes and material refs.
8. Add assign-individual-material-from-Book action.
