...
- Current conflict winner for Source Delivery cost/transport: [`prd-book-based-interactive-activity-runtime-and-assembly.md`](prd-book-based-interactive-activity-runtime-and-assembly.md) plus [`canonical-task-overrides.json`](canonical-task-overrides.json), approved 2026-07-14.
...
- Canonical task owners: current Components 01–08 in this directory.
- Evidence snapshots: [`recovered/`](recovered/). Never use recovered checkbox state as current status.
- Audit/evidence: [`implementation-audit.md`](implementation-audit.md), [`reconciliation-ledger.md`](reconciliation-ledger.md). These classify; they do not close task rows.

## Operating rule

Implement packet by packet. Within current packet, verify interfaces, allow current-packet consumers, complete all required proof, reconcile canonical task rows, and close packet. Do not carry unreviewed behavior or proof debt across packet boundaries. A remote/deployment action without separate approval is a blocker, not a reason to skip a row.
...
Packet names are navigation only. Exact task IDs and wording in Component files control scope. Amendment packet order controls where this pointer conflicts with the 9e master.
...
Current packet is P2. Resume only from dependency-ready open rows in Components 02–03 after their corrected wording passes governance. Do not enter P3 until the P2 exit contract, including no-cost authenticated document streaming, published-only mapping producer proof, student-safe/private-authority denial boundaries, recovery, and approved remote/browser evidence, is accepted. P2 does not issue Solo/Homework entitlements and does not need a successful entitled student launch to close.
