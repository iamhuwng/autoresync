...
is the historical source review. The [planner decisions/current work](../../notification-recovery-route-boundary.md#planner-decisions-and-current-work-2026-09-27)
record approved app ownership and remaining implementation/browser gates.
...
Manual reminder, session, and THCS callers now use the shared producer after
ordinary app-owned source/intent saves. Four other clients still call Worker
product mutations pending coordinated owner changes; that coupling is not
necessity proof.

| Producer path | Migration owner | Family | Current status |
|---|---:|---|---|
| `src/components/practice/THCSPracticeView.tsx` | #97 | thcs-practice | App result save; shared delivery wake |
| `src/components/thcs-editor/THCSHomeworkAssignDialog.tsx` | #97 | thcs-practice | App assignment save; shared delivery wake |
| `src/components/thcs-grading/InlineWritingGrader.tsx` | #96 | result | Unapproved Worker manual-grade mutation caller |
| `src/components/thcs-student/THCSTestLayout.tsx` | #97 | thcs-practice | App result save; shared delivery wake |
| `src/hooks/monitor/useMonitorControls.ts` | #97 | session | App source/event/intent save; shared delivery wake |
| `src/pages/TeacherHomeworkDetailPage.tsx` | #95 | deadline | App reminder save; shared delivery wake |
...
| `src/services/sessionManager.js` | #97 | session | App source/event/intent save; shared delivery wake |
...
Seventeen unique producer paths contain 18 shared wake calls in 12 paths. The
additional `homeworkManager.ts` path creates intents without waking delivery.
Manual reminder (two calls), session (three), and THCS (three) consolidation is
implemented; representative enabled reminder browser verification passed. Source
owners and server authority resolvers retain their existing contract.
...
All ordinary wakes now use the shared source-side client after their ordinary
app/Firebase save. Remaining action clients call a Worker-owned action. Class membership is the
...
grading have recorded planner ownership decisions with coordinated
implementation/proposal gates still open. Recipient and message
