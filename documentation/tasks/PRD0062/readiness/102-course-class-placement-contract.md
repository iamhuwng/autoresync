...
The enrollment fact is produced only by the future trusted `CourseEnrollmentAuthorityPort.transitionDirectCourseEnrollment` contribution through #59. It atomically transitions the legacy direct-Course random-ID record and the exact authority child, rejects class-based transitions and duplicate active records, records a revision/legacy ID, expires from trusted time, and makes unenrolment revoke-only. Migration is bounded and explicitly approved; rollback is deny-only; restore/replay has no fan-out outside an approved recovery phase. #118 composes service-identity-only rules.
