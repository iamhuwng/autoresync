...
The enrollment fact is produced only by the future trusted `CourseEnrollmentAuthorityPort.transitionDirectCourseEnrollment` contribution through #59. It writes one deterministic `course_enrollments/course:{courseId}:student:{studentId}` record with conditional CAS, rejects class-based transitions and duplicate active rows, records a revision, expires from trusted time, and makes unenrolment revoke-only. A bounded explicit activation gate migrates eligible direct-Course random-ID rows; Class rows remain untouched. Rollback is deny-only; restore/replay has no fan-out outside an approved recovery phase. #118 composes service-identity-only rules.
