...
- **#102 / 42A**, **#103 / 42B**, **#104 / 42C**, and **#107 / 39D** are
  `IMPLEMENTED_LOCAL_VERIFIED` at their bounded local proof boundaries. **#106
  / 44** is integrated but `IMPLEMENTED_INTEGRATION_FIX_REQUIRED`. The current
  frontier is a canonical Activity schema/type compatibility investigation for
  #106, followed by the smallest accepted repair and then #108 readiness; #127
  source conformance remains an independent acceptance-artifact blocker.
...
#103, #104, and #107 have bounded local proof. #106 is integrated but remains
`IMPLEMENTED_INTEGRATION_FIX_REQUIRED`: its public reference/fork code imports
retired `BookActivity*` types and absent Activity schema helpers, with tests
currently surfacing first at `normalizeActivityRevision`. No emulator,
authenticated-browser, generated/active-rules, deployed, canary, activation,
pilot, or Full V1 claim is made. #106 must first establish the canonical
schema/type compatibility boundary and accept the smallest repair; #108 is the
next serialized readiness lane only after that gate passes.
...
| **#106 / 44** — §10 Content Catalog reference/fork and §11 public safety | `IMPLEMENTED_INTEGRATION_FIX_REQUIRED` | Commit `a92c8158` contains the public reference/fork vertical, but it imports retired `BookActivity*` types and absent Activity schema helpers; aggregate tests surface first at `normalizeActivityRevision`. | Establish the exact compatibility mapping to the canonical Activity schema/type model, review it, then implement the smallest accepted repair and rerun the existing reference/fork negatives. Do not presume an import-only fix, redesign the vertical, or claim #134 deployed proof. | S0 → #107, #118, #134 | Compatibility review and existing domain/Worker tests must pass before local verification; deployed proof remains deferred. |
| **#107 / 39D** — §§9–10 Course/Class/public impact adapters | `IMPLEMENTED_LOCAL_VERIFIED` | Commits `17a42a0a`, `b5c33438`, and `16567771` implement the shared bounded read-only discovery engine and Course/Class/public adapters with focused no-mutation and identity-boundary proof. | No snapshot persistence or mutation belongs here. #108 consumes the adapter registry after the bounded #106 integration fix is accepted. | #102 + #103 + #106 → #108 | Bounded local adapter/read-budget/privacy/stale/no-mutation proof; no deployed claim. |
| **#108 / 39C** — §9 authoritative affected-context snapshot and review | `NEXT_READINESS_LANE_PENDING_106_FIX` | #42 classification, #101 Solo/Homework discovery, and #107's adapter registry are implemented locally; no durable complete-context snapshot or teacher review exists. | After #106's canonical schema/type compatibility repair is accepted, freeze and implement immutable fingerprint/TTL snapshots and indexes, safe Worker projection, and teacher review UI. Mutation remains with #109–#117. | accepted #106/#107 plus closed #67/#68/#71/#101 → #109, #112–#115, #118 | Snapshot/repository/CAS/TTL tests; all-context completeness and stale/uncertain denial; read-only Worker proof; teacher browser review; fragment/local rollback. |
...
| **#135 / 52A** — §§5, 11, 14 no-cost baseline harness | `PARTIAL_STANDING_LANE_BLOCKED_FINAL_EVIDENCE` | Provider capacity probes and scattered ticket measurements exist; the current probe can persist a reconciled capacity ledger, so it is not yet a complete refusal-safe read-only baseline. | Build true read-only mode (or explicitly bounded owned-write semantics), demand model, refusal/redaction logic, and account/bucket/Worker baseline early. Final representative measurement and headroom/billing decision wait for stable routes and #118. | early safe preparation; final #118 + closed #44/#49/#59/#76 → #136 | Harness unit/safety/no-unintended-write tests; read-only baselines; final measured Worker/B2/Firebase request/byte/latency envelope, zero billed use, and required headroom artifact. |
...
2. **Placement/reference verticals:** #102 and #103 are locally verified. #106 is integrated but gated on canonical Activity schema/type compatibility review and the smallest accepted repair; it does not rewrite #102 authority.
3. **Context integration:** #104 launch composition and #107 read-only discovery are locally verified. #108 waits for accepted #106 compatibility before beginning its serialized snapshot lane.
...
5. **Standing lanes:** #118 may build composer/manifest/conflict checks and ingest fragments continuously; #135 may build its read-only demand model early. Neither may claim final evidence early.
...
- **S1 — placement/reference integration:** #102/#103 are locally verified; #106 remains unaccepted until its canonical Activity compatibility repair passes review and focused proof.
- **S2 — launch and adapters:** #104/#107 are locally verified. After #106 acceptance, #108 freezes one immutable complete-context snapshot.
