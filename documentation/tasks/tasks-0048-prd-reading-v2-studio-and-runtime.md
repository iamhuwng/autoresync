...
- [ ] 10.0 Correct student Reading V2 runtime to ready-use IELTS test-taking behavior
  ...
  - [x] 10.10 Record any remaining first-release blockers separately, especially trusted submission endpoint/configuration, without marking student runtime behavior complete if the blocker prevents submit/result readiness.
    ...
    - **Resolved Blocker Update 2026-05-11:** Production now configures `VITE_READING_V2_SUBMISSION_ENDPOINT` to the deployed Cloudflare Worker route. If the endpoint is absent, the client fails closed instead of deriving the undeployed Firebase Cloud Functions URL.
- [x] 11.0 Port the full documented V1 student Reading UI contract into the V2-native runtime
  ...
  - [x] 11.7 Verify the live student view on port 5173 at desktop/tablet and phone widths, compare against the documented V1 contract, and record remaining blockers separately from completed runtime behavior.
    ...
    - **Resolved Blocker Update 2026-05-11:** Trusted submission is now configured for production through the deployed Cloudflare Worker endpoint. The remaining release gate is authenticated live submit/result verification, not endpoint availability.
- [x] 12.0 Apply foundational Reading V2 runtime/session correctness fixes from review
  ...
  - [x] 12.10 Verify focused Reading V2 runtime/projection/import and host page tests, targeted UTF-8 checks, filtered touched-file TypeScript scan, and the active student browser route on port 5173.
    ...
    - **Resolved Blocker Update 2026-05-11; deprecation update 2026-05-24:** Production now has a configured Worker endpoint. Cloud Functions are off-limit for new Reading V2 work; local dev should use an explicit Worker/local trusted-backend endpoint rather than relying on a Functions emulator fallback.
- [x] 13.0 Complete foundational trusted Reading V2 submit endpoint setup
  ...
  - **Done Criteria:** A trusted backend exports or routes `POST /api/reading-v2/submit` (currently the existing Cloudflare Worker; Firebase Functions are deprecated historical wrappers, not a production fallback); production uses `VITE_READING_V2_SUBMISSION_ENDPOINT` to target that backend and fails closed if the explicit endpoint is missing; tests cover request parsing, canonical scoring, answer-key-free review payload, existing result/index writes, live-session completion updates, and client endpoint/auth behavior; backend typecheck/build, production build, and targeted UTF-8 checks pass.
  ...
  - [x] 13.4 Configure deployment surfaces for a trusted submit route. Historical Firebase Functions wiring exists, but current production alignment uses the existing backup Cloudflare Worker instead of Firebase Cloud Functions; new Cloud Function work is off-limit.
  ...
  - [x] 13.7 Add tests for trusted request parsing/scoring/persistence plan and client explicit-endpoint, dev-emulator, auth, and failure behavior.
  ...
  - [x] 13.9 Verify trusted-submit backend checks, focused Vitest suites, production build, and targeted UTF-8 checks.
    ...
    - **Deployment Finding:** `curl.exe -i -X OPTIONS https://us-central1-temp-a1437.cloudfunctions.net/readingV2Submit ...` returned `404 Page not found`, `cmd /c npx firebase functions:list --project temp-a1437` returned `No functions found`, and `cmd /c npx firebase deploy --only functions:readingV2Submit --project temp-a1437 --dry-run` failed because `temp-a1437` must be upgraded to Blaze before Cloud Build/Artifact Registry APIs can be enabled. Later alignment moved the production path to the existing Cloudflare Worker route. As of 2026-05-24, Firebase Functions are deprecated historical implementation code and off-limit for new Reading V2 work.
    - **Alignment Note 2026-05-11:** `origin/main` now points production Reading V2 trusted submission at `https://r2-backup-worker.iamhuwng.workers.dev/api/reading-v2/submit`; the Cloudflare Worker and Firebase Hosting `kahut1` deploys are complete; Firebase Hosting no longer rewrites `/api/reading-v2/submit` to the undeployed Cloud Function; unauthenticated Worker submit returns `403` with CORS as expected. Authenticated production Worker verification is complete: solo-practice submit wrote `reading-v2-result-938044f7-2529-4cc4-be2c-447c8f0a09d7` and live-session verification session `CDXV2T` wrote `reading-v2-result-1e6af2cf-6f43-4941-a79a-e13abab55e25`, both scoring `40/40` with canonical result/index records present in RTDB.
