...
- [x] Implement the minimal approved Reading V2 audit writer before any state-changing archive, restore, repair, remove, or duplicate-decision service can be accepted.
...
- [x] Add audit path rules before archive/restore service parent acceptance:
...
- [x] Add security tests before archive/restore service parent acceptance:
...
- [x] Parent acceptance: audit service and audit rules are test-covered before any later phase can claim state-changing archive/restore/repair behavior complete.

## Phase 1B - Duplicate Index Foundation

- [x] Use the approved deterministic duplicate formula from PRD FR-DUP-4A:
...
- [x] Add and secure the owner-scoped current-material duplicate index path `reading_v2/duplicate_indexes/passages_by_owner/{ownerId}/{passageMaterialId}`.
...
- [x] Add duplicate-index rules and tests before PRD-0052 final acceptance:
...
- [x] If no existing duplicate service exists, add `src/services/reading-v2/readingV2PassageDuplicateGuard.service.ts`.
...
- [x] Parent acceptance: PRD-0052 Part 2 can consume the duplicate guard/index for auto-split publish without waiting for PRD-0054 repair UI phases.
...
- [x] Start this phase only after Phase 2 archive/restore data services and Phase 3 broken-ref invalidation ownership pass parent acceptance.
- [x] Update Teacher Lobby UI:
...
- [x] Update `src/components/modern/materialListAdapter.js` and `src/components/modern/MaterialListRow.jsx` only if row metadata/actions are owned there.
...
- [x] Update `src/components/modern/materialListAdapter.test.js` and `src/components/modern/MaterialListRow.test.jsx`.
...
- [x] Parent acceptance: teacher can remove a Reading Passage from active library, see it in Archive subtab, restore it, and affected master/Book broken-ref summaries are updated by service owners, not listing/card reads.

## Phase 5 - Master Repair UI In Edit Test Modal

- [x] Start this phase only after PRD-0052 Part 2 Phase 8 marks the dependency ready in its findings file, and the master Edit Test Modal tests pass.
- [x] Edit `src/components/reading-v2/master/ReadingV2MasterEditModal.tsx`.
...
- [x] Add `src/components/reading-v2/master/ReadingV2MasterRepairPanel.tsx` and `.test.tsx` if the repair UI is too large for the modal body.
...
- [x] Parent acceptance: a teacher can open a broken master, repair or remove all broken refs, confirm numbering changes, and publish a repaired master without using full-test Studio.

## Phase 6 - Book Broken Reference UX And Repair

- [x] Update Book service validation:
...
- [x] Update Book material services:
...
- [x] Parent acceptance: Books with broken Reading Passage refs are visible, fixable in the existing modal, and blocked from unsafe future use until repaired.

## Phase 7 - Duplicate Warning Surfaces

- [x] Start this phase only after Phase 1B duplicate index foundation passes and PRD-0052 Part 2 Phase 2B integration status is recorded.
- [x] Do not reimplement the duplicate formula, duplicate index, or duplicate guard service here; consume `readingV2PassageDuplicateGuard.service.ts`.
- [x] Do not implement duplicate detection from broad canonical content scans or answer keys.
- [x] Integrate warning surfaces:
...
- [x] Add UI tests in the owning components after inspection:
...
- [x] Parent acceptance: duplicate warning surfaces consume the Phase 1B index/service, stay non-blocking at 80 percent or higher, and use no unsafe broad content hydration.

## Phase 8 - Assignment, Publish, Runtime, And Result Safety

- [x] Update assignment guards:
  - `src/services/reading-v2/readingV2PassageHomework.service.ts`
  - `src/services/reading-v2/readingV2PassageHomework.service.test.ts`
...
- [x] Parent acceptance: future unsafe use is blocked, old frozen learning evidence remains available, and no result changes when source materials are archived/restored/repaired.

## Phase 9 - Audit, Observability, Security Rules Final Sweep

- [x] Verify and extend the approved Reading V2 audit path from Phase 1A and `documentation/architecture/reading-v2-audit-trail.md`; do not introduce a second audit writer.
...
- [x] Required observability-only events, not append-only audit events unless super-admin audit policy later expands:
  - broken master ref viewed
  - broken Book ref viewed
  - duplicate warning shown
- [x] Update `src/config/featureRegistry.ts` and `src/config/featureRegistry.test.ts`.
...
- [x] Update `src/__tests__/security/readingV2FirebaseRules.test.ts`.
...
- [x] Update `src/__tests__/security/materialCatalogFirebaseRules.test.ts` for Book broken-ref rows and archive metadata if material catalog rules are touched.
- [x] Update `firestore.rules` and `src/__tests__/security/homeworkFirestoreRules.test.ts` only if homework assignment schema changes.
- [x] Parent acceptance: every new write path has a rule test, and every visible action has registry coverage.

## Phase 10 - Documentation And PRD-0052 Feedback

- [x] Update `documentation/architecture/reading-v2-material-publish-and-passage-library.md`.
  - Add archive/restore lifecycle.
  - Add broken-ref reason code contract.
  - Add frozen assignment/result safety note.
- [x] Update `documentation/architecture/teacher-materials-listing-and-diagnostics.md`.
  - Add Archive subtab list behavior.
  - Add active-list exclusion rule.
  - Add broken-ref badge source rule.
- [x] Update `documentation/architecture/teacher-materials-list-view-contract.md`.
  - Add row action label `Remove from library`.
  - Add restore action placement.
  - Add broken-ref row state.
- [x] Update `documentation/architecture/book-editor-authoring-modal-architecture.md`.
  - Add broken-ref Book editor behavior.
  - Preserve modal-first, 3-tab contract.
- [x] If PRD-0052 Part 2 implementation exposed a different modal/service path than this tasklist expected, update this tasklist and findings before coding the dependent phase.
- [x] Parent acceptance: architecture docs match implemented archive, restore, repair, and guard behavior.
...
- [x] Audit and duplicate-index foundations:
...
- [x] Archive/restore data and library:
...
- [x] Broken references and master repair:
...
- [x] Book repair:
  - `cmd /c npx vitest run src/pages/BookEditorSmokePage.test.tsx src/services/materialCatalog/bookValidation.service.test.ts src/services/materialCatalog/bookEditor.service.test.ts src/services/materialCatalog/materialBooks.service.test.ts src/components/modern/BookCardGrid.test.jsx src/components/books/BookEditorModal.test.tsx src/components/books/BookEditorWorkspace.test.tsx src/components/books/BookNodeTree.test.tsx src/components/books/BookMaterialPicker.test.tsx --reporter=basic`
- [x] Duplicate warning surfaces:
  - `cmd /c npx vitest run src/components/reading-v2/studio/ReadingV2DuplicateWarningPanel.test.tsx src/components/reading-v2/studio/ReadingV2StudioModalAdapter.test.tsx src/pages/ReadingV2StudioSmokePage.test.tsx --reporter=basic`
  - UI tests prove warning shown, non-blocking status, `Use existing`, `Create new anyway`, `Restore and use`, and unsafe payload non-exposure. Phase 7 was not claimed with service tests only.
- [x] Assignment/runtime/result/publish:
...
- [x] Use the in-app Browser or Playwright first when it can reach the local dev server and complete dev quick-login.
...
- [x] Click the subtle settings icon in the bottom-right corner. Teacher session was already authenticated; Student proof used explicit logout, settings reveal, and Student quick-login.
- [x] Use the `Teacher` quick-login button. Teacher session was already authenticated; no manual credentials were entered.
- [x] Teacher proof 1: `/lobby` -> Reading Passage filter -> active tab. Remove a Reading Passage from library. Confirm it disappears from active list and appears in Archive subtab. Packet 9 live disposable passage proof archived `packet9-live-20260610151227-passage`; screenshots `13-live-active-before-archive-5173.png`, `14-live-archive-dialog-5173.png`, `15-live-archive-row-after-rule-fix-5173.png`, `18-live-active-after-successful-archive-5173.png`.
- [x] Teacher proof 2: Archive subtab -> restore the passage. Confirm it returns to active list and can be selected by add-existing picker. Packet 9 live proof restored `packet9-live-20260610151227-passage` as private; screenshots `16-live-restore-dialog-5173.png`, `17-live-restored-active-5173.png`.
- [ ] Teacher proof 3: remove a passage used by a master. Open affected master Edit Test Modal. Confirm broken-ref warning, exact reason, repair actions, and blocked publish until repair. Packet 7 automated tests cover this; live visible masters had no broken refs.
- [ ] Teacher proof 4: repair broken master by choosing an existing passage, confirm numbering review, publish repaired master, reopen and confirm warning gone. Packet 7 automated tests cover this; live visible masters had no broken refs.
- [x] Teacher proof 5: remove a passage used by a Book. Open Book editor modal. Confirm broken node warning, replacement/removal actions, and 3-tab modal contract. Packet 8 automated tests cover broken refs; live browser proof used existing Book modal and confirmed Content tab repair region without mutating dev data.
- [x] Teacher proof 6: trigger duplicate warning in single-passage publish or creation flow with approved duplicate fixture. Confirm warning is visible, non-blocking, and tracked.
- [x] Teacher proof 7: try assigning archived/broken material. Confirm assignment is blocked with clear error. Packet 9 Book smoke proof shows broken selected material `Assign selected` disabled; live archive proof also confirms archived passage leaves active assignment-capable list after successful re-archive.
- [x] Student proof: use `Student` quick-login, open existing homework assigned before archive, complete or review it, and confirm frozen assigned/result content still works. Packet 9 disposable homework/result proof used `packet9-live-20260610151227-hw-launch`, `packet9-live-20260610151227-hw-result`, and `packet9-live-20260610151227-result`; screenshots `20-student-homework-frozen-cards-5174.png`, `21-student-frozen-runtime-5174.png`, `22-student-frozen-result-panel-5174.png`.
- [x] Responsive proof for touched teacher modal/list UI: 1366 px, 848 px, 375 px, and 320 px widths. Confirm no horizontal overflow, no overlapping text, and primary actions remain visible. Packet 9 checked lobby Reading Passage list, Book broken-ref smoke, and duplicate warning smoke at required widths; screenshot paths are in findings.
...
- [x] Archive is reversible; do not use hard delete for Reading Passage snapshots or versions.
- [x] Restore must reconstruct active indexes from canonical metadata, not from stale UI row state.
- [x] Do not mutate assignment-pinned projections or completed result snapshots during archive, restore, repair, or duplicate checks.
- [x] Do not let active lists, add-existing pickers, homework assignment pickers, or launch flows use archived materials.
- [x] Do not compute broken-ref summaries in student launch paths by writing back to catalog state.
- [x] If repair flow needs PRD-0052 modal APIs that do not exist, stop and complete PRD-0052 first.
- [x] If duplicate guard requires broad canonical content scans, stop and redesign indexing before implementation.
- [x] If `reading_v2/audit_events/{eventId}` cannot be secured as append-only with super-admin read and unsafe-field rejection, stop before implementing state-changing audit-required flows.
- [x] Keep PRD-0054 commits separate from unrelated dirty work already present in the worktree.

## Ambiguity Flags

- [x] PRD-0054 audit path is approved as `reading_v2/audit_events/{eventId}`. Implementation must follow `documentation/architecture/reading-v2-audit-trail.md`.
- [x] PRD-0054 duplicate threshold and formula are approved. Implementation must use the hashed-shingle Sorensen-Dice formula from PRD FR-DUP-4A.
- [x] PRD-0054 duplicate index path is approved as `reading_v2/duplicate_indexes/passages_by_owner/{ownerId}/{passageMaterialId}`. Use current material rows with `currentVersionId`; do not create version-row indexing in V1.
- [x] PRD-0054 names delete/remove master behavior but does not define hard-delete retention. This tasklist uses soft archive/remove semantics unless existing architecture explicitly approves hard delete.
- [x] PRD-0054 does not request a normal teacher restore surface for removed masters. Do not add one in V1 without a separate approval.
- [x] V1 has no super-admin archive/restore UI and no super-admin audit-management UI. Future UI requires a separate PRD.
- [x] Active listing broken-ref badges require a safe index owner. If no owner exists, modal-only detection is acceptable only when documented as deferred listing badge work.
- [x] Book repair depends on the current Book editor modal contract. Do not replace the modal-first Book editor with a route page.

## Final Acceptance Criteria

- [x] Reading Passage active lists exclude archived passages.
- [x] Archive subtab lists archived passages with restore action.
- [x] Restore returns a valid passage to active lists and pickers.
- [x] Archive/restore never deletes immutable snapshots or published versions.
- [x] Current masters and Books with archived/deleted/missing refs show broken-ref state. Packet 9 tests and Book smoke proof cover Book; master repair tests cover current masters.
- [x] Broken current masters cannot be assigned, launched, or published until repaired.
- [x] Broken Books cannot be used in unsafe future flows until repaired.
- [x] Master repair happens in Edit Test Modal from PRD-0052, not full-test Studio.
- [x] Book repair happens in existing Book editor modal and keeps the 3-tab contract.
- [x] Existing assignments and completed results remain frozen and reviewable after archive, restore, delete/remove, and repair.
- [x] Duplicate warning uses approved deterministic formula, warns at 80 percent or higher, and does not expose answer/canonical payload.
- [x] Security rules cover every new write/read path.
- [x] Observability covers every new visible action.
- [x] Architecture docs match implemented behavior.
- [x] Targeted tests, browser proof, UTF-8 check, and `git diff --check` pass.
- [x] Browser proof records browser surface, viewport, URL, ids, expected result, actual result, and screenshot/trace path for each archive, restore, repair, duplicate-warning, assignment-block, and frozen-result step.
