...
**P2 source correction:** the planner found that fast inbox backend failures in class/homework scheduled handlers could return a successful RPC because the handlers broke their loops. Both shared scheduled functions now reject with family-specific backend error codes, retaining the unconfirmed `retrying`/attempt 2 claim and the read-only interrupted recovery path. A sixth runtime file, `homework-retry.ts`, is explicitly authorized. Previous CI remains historical evidence; the final affected Workerd run now passes 38/38, including both failures reproduced red before the fix. [CI 36265610109](https://github.com/iamhuwng/autoresync/actions/runs/36265610109) passed on `136634e919897a2529d74d3e1eb23a1856398e78` across app notification services, RTDB/Firestore authority, atomic result/index deletion, Workerd actions and candidate/recovery bundles. The [read-only preflight](notification-recovery-executor-preflight.md) verifies the existing active version and no notification DO migration/binding; actual plan/remaining capacity are unknown (billing 403, empty analytics datasets). Deployment/migration/backlog seeding remain **HELD**. Visible admin recovery/suppression remains a separate open gate.
...
| `cloudflare/src/upload-worker/notifications/homework-retry.ts` | One-line backend-failure rejection correction alongside class retry |
