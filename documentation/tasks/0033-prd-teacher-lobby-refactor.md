...
> Status note, updated 2026-07-07: this PRD is a historical extraction/refactor source. Its original `useTeacherTests` data-loading requirement used `queryOptimizer.getAllTests()` because the refactor preserved old behavior. That loading model is now obsolete. Current Teacher Materials loading is governed by `documentation/architecture/teacher-materials-listing-and-diagnostics.md`: My Content reads `material_catalog/material_summary_indexes/v1/by_owner/{teacherId}`, Public Library reads `material_catalog/material_summary_indexes/v1/by_visibility/public`, and `/tests`/`getAllTests()` is runtime or legacy compatibility only, not the listing authority.
