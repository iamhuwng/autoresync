...
- `documentation/architecture/changelog/result-view-permission-matrix.md` was audited as already structurally complete for `11.2`; this slice adds the explicit same-change-set rule there so the governance bundle enforces itself.
- `documentation/architecture/changelog/result-view-fr-closure-matrix.md` was upgraded from a coarse FR summary into a locked by-layer PRD scenario matrix. Each later-phase scenario now points to concrete service, writer-flow, page/component, security/rules, analytics, and route-entry/navigation anchors, and the matrix records the current state as `passing` based on the previously completed scoped verification runs.
...
- The current implementation materially narrows PRD-0040 governance artifacts: `documentation/architecture/changelog/result-view-permission-matrix.md` and `documentation/architecture/changelog/result-view-fr-closure-matrix.md` now read as PRD-0041 Phase 1 teacher-visibility docs rather than the broader all-surface result-view verification pack PRD-0040 originally defined. Runtime shell separation is mostly preserved, but the living-doc governance layer has drifted.
...
- Reduced the PRD-0040 governance drift in the living docs by rewriting the lead scope statements in `documentation/architecture/changelog/result-view-permission-matrix.md` and `documentation/architecture/changelog/result-view-fr-closure-matrix.md` so they explicitly remain unified PRD-0040 + PRD-0041 governance artifacts rather than 0041-only replacements.
...
## 2026-05-10 homework-result visibility repair amendment

- Earlier unresolved-report and result-write-path findings remain historical context. They do not explain the current Teacher Homework Detail `Access Revoked` failure class.
- The current failure class was a homework result ownership misclassification: generic homework `context.source.submissionId` was treated as a Writing submission id, causing `visibility.sourceType: "writing_submission"`, `ownershipResolved: false`, `unresolvedReason: "writing_submission_not_found"`, and no teacher-readable `/test_results/{resultId}` rule path.
- Canonical repair is now recorded in `documentation/architecture/changelog/homework-result-visibility-repair.md`: normal Reading/Listening homework results resolve through `context.assignment.homeworkId` -> `homework_assignments/{homeworkId}.createdBy`; only explicit Writing identifiers may load `writing_submissions/{submissionId}`.
- Historical unresolved rows must be repaired through `ensureResultVisibility()` / `rebuildTeacherResultIndexes()` so teacher indexes and RTDB read permissions converge on the same canonical visibility. Modal-local fallbacks and one-off live row edits are obsolete for this failure class.
