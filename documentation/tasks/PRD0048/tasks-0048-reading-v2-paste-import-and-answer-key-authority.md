...
Implementation note 2026-05-13: External-AI prompt schema now explicitly includes top-level `answerKeyText` as the complete numbered teacher-key row carrier. `answerKeyAudit` remains a review/audit helper only and is not the required binding source. This retires the older implicit contract where external AI could fill only `questions[].answer` plus `answerKeyAudit`; Studio normalization prefers the separate teacher/effective `answerKeyText` rows and then binds them into canonical `scoringRule.acceptableAnswers`. Files touched: `src/services/reading-v2/readingV2ExternalAiPrompt.service.ts`, `src/services/reading-v2/readingV2ExternalAiPrompt.service.test.ts`, and this task list. Verification: focused prompt-contract Vitest passed as part of the Auto answer-key hotfix.
