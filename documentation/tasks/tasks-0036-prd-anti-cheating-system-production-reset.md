...
- Rollout caveat: existing legacy tests may not yet have `student_safe_tests/{testId}` populated. Production rollout must backfill that node for the active test catalog or ensure affected tests are re-saved before enabling the new solo/homework safe loader broadly.
...
- `documentation/tasks/0036-anti-cheat-runtime-contracts.md` documents the implementation-level additions that are easy to miss in the PRD: student-safe payload nodes, session payload caching, refresh/reset coordination timestamps, and session cleanup rules.
