...
export const writingTestTools: ToolRegistration[] = [
    ...
    {
        category: 'teacher',
        activeRoutes: ['/teacher/grading', '/teacher/grading/*'],
        allowedRoles: ['teacher', 'super_admin'],
        tool: {
            name: 'navigate_writing_grading_queue',
            description: 'Navigate to the Grading page with the Writing tab selected to see pending submissions.',
            inputSchema: {
                type: 'object',
                properties: {},
            },
            execute: async () => {
                window.history.pushState({ tab: 'writing' }, '', '/teacher/grading');
                window.dispatchEvent(new PopStateEvent('popstate'));
                return { content: [{ type: 'text', text: 'Navigated to Grading page (Writing tab).' }] };
            },
        },
    },
    {
        category: 'teacher',
        activeRoutes: ['/teacher/grading'],
        allowedRoles: ['teacher', 'super_admin'],
        tool: {
    ...
    },
    {
        ...
        allowedRoles: ['teacher', 'super_admin'],
        tool: {
            name: 'open_writing_grading_modal',
            description: 'Open the Writing Grading modal for a specific submission. Finds the submission card in the IELTS Writing tab and clicks its Grade button.',
            inputSchema: {
                type: 'object',
                properties: {
            ...
            execute: async ({ submissionId }) => {
                try {
                    // Find the Grade button for this submission by matching the card
                    const cards = document.querySelectorAll('[data-submission-id]');
                    for (const card of Array.from(cards)) {
                        if (card.getAttribute('data-submission-id') === submissionId) {
                            const btn = card.querySelector('button');
                            if (btn) { btn.click(); }
                            return { content: [{ type: 'text', text: `Opened grading modal for submission ${submissionId}.` }] };
                        }
                    }
                    // Fallback: click any Grade button that navigates to this submission
                    const btns = Array.from(document.querySelectorAll('button'));
                    const gradeBtn = btns.find(b => b.textContent?.includes('Grade'));
                    if (gradeBtn) {
                        gradeBtn.click();
                        return { content: [{ type: 'text', text: `Opened grading modal (first available submission).` }] };
                    }
                    return { content: [{ type: 'text', text: 'No grading submissions found. Is the Writing tab active?' }], isError: true };
                } catch (error: unknown) {
                    const message = error instanceof Error ? error.message : 'Unknown error';
                    return { content: [{ type: 'text', text: `Error: ${message}` }], isError: true };
                }
    ...
    },
    {
        ...
        tool: {
            ...
            description: 'Get the current state of the grading modal: active task, scores per criterion, overall band, annotations count, void status, and essay preview.',
            inputSchema: {
                type: 'object',
                properties: {},
            },
            ...
            execute: async () => {
                try {
                    // Check if modal is open by looking for the modal overlay
                    const modalOverlay = document.querySelector('.wgm-overlay');
                    if (!modalOverlay) {
                        return { content: [{ type: 'text', text: 'Grading modal is not open.' }], isError: true };
                    }
                    const titleEl = modalOverlay.querySelector('.wgm-title-area h2');
                    const studentName = titleEl?.textContent || '';
                    const activeTab = modalOverlay.querySelector('.wgm-tab--active')?.textContent || 'Task 1';
                    const overallBand = modalOverlay.querySelector('.wgm-overall-band')?.textContent || '—';
                    const annotationCount = modalOverlay.querySelectorAll('.annotation-group').length;

                    const scoreButtons = modalOverlay.querySelectorAll('button[style*="border: 2px"]');
    ...
    },
    {
        ...
        tool: {
            ...
            description: 'Click the Save Draft button in the grading modal to save current grading progress without submitting.',
            inputSchema: {
                type: 'object',
                properties: {},
            },
            execute: async () => {
                try {
                    const modal = document.querySelector('.wgm-overlay');
                    if (!modal) return { content: [{ type: 'text', text: 'Grading modal is not open.' }], isError: true };
                    const saveBtn = modal.querySelector('.wgm-btn-save') as HTMLButtonElement;
    ...
    },
    {
        ...
        tool: {
            ...
            description: 'Click the Submit Grading button in the grading modal to finalize and submit. Will validate that at least one task has all 4 criteria scored.',
            inputSchema: {
                type: 'object',
                properties: {},
            },
            execute: async () => {
                try {
                    const modal = document.querySelector('.wgm-overlay');
                    if (!modal) return { content: [{ type: 'text', text: 'Grading modal is not open.' }], isError: true };
                    const submitBtn = modal.querySelector('.wgm-btn-submit') as HTMLButtonElement;
