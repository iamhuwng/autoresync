...
| `/guest-results` via `GuestResultsPage` | public route | none | RTDB `guest_results/{guestName}` | top-level read requires `auth != null`; child write is unrestricted | `GuestResultsPage.test.tsx` | mismatch (accepted) | Public route vs. auth-required backend read is an accepted mismatch (Finding F-5.3b). CTA routes corrected to `/` in Task 5.3. |
| `/profile/complete` guest claim | authenticated users | `PrivateRoute` | RTDB `guest_results/{guestName}` read and `test_results/{userId}` write | claim path writes to canonical `test_results/{userId}` with compatibility metadata; guest node writes are open | `ClaimResultsModal.test.tsx` | compatibility (accepted) | Storage decision: keep current compatibility path. Claim writes to canonical path with additive `claimedAt`/`claimedFrom` metadata (Finding F-5.2a). |
