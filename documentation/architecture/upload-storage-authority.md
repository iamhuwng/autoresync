...
- current authoring UI now uses the canonical upload-session path and exact temporary-identity cleanup for local replacement/remove/unmount handling; editor heartbeat, multi-tab edit-turn lease aggregation, deployment, and scheduled activation remain open;
- legacy callers without canonical bridge metadata/committer remain a compatibility risk; the recovered canonical path fails closed instead of treating an expiring temp URL as durable;
- Listening test deletion removes the RTDB record but does not remove its R2 audio;
- abandoned/cancelled upload cleanup now has local authoring event wiring and a trusted executor implementation, but neither the scheduled sweep nor live cleanup behavior is deployed or activated;
...
- the 2026-08-23 recovery integration restores a local upload-session cleanup executor and scheduled-event hook, but canonical Wrangler configuration keeps the sweep explicitly disabled. The dormant implementation is not deployed/current cleanup proof. Editor-tab heartbeat and multi-tab edit-turn lease aggregation remain unimplemented. Activation requires those lifecycle semantics, direct sweep checkpoint/lease/retry/limit/final-record tests, reconciled cutoff and rollout authority, emulator-backed rules proof, restore/deletion proof, and separate deployment/remote-mutation approval;
