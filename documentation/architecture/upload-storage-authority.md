...
Listening authoring uploads new canonical audio through `r2StorageService.uploadListeningAuthoringAudio(...)` and keeps older generic temp upload compatibility through `r2StorageService.uploadAudioReplacement(...)`. Listening save/publish still preserves the R2 temp-to-permanent lifecycle through `src/services/listeningTestStorage.ts`.
...
Current deployed abandoned-upload cleanup truth recorded on 2026-07-02:

Canonical current temp-cleanup authority: `documentation/architecture/listening-temp-upload-cleanup-authority.md`.

- Production Worker `r2-upload-signer` version `fc4898f5-6acb-4df0-9ee1-bcbebec63d12`, deployment `5f004ba2-a482-4312-bb5c-ae44d4ae030e`, serves `POST /cancelListeningUploadSession` at 100%. The route authenticates with Firebase, derives temp keys from `media_asset_upload_sessions/{ownerId}/{uploadSessionId}`, validates canonical `temp/listening/{ownerId}/{uploadSessionId}/{assetId}-{sanitizedFileName}` keys, denies committed/committing sessions, checks durable references across `media_assets`, Listening drafts/versions, tests, results, assignments, and sessions, writes `cleanup-queued` before any trusted R2 delete attempt, deletes only unreferenced temp objects through the trusted R2 binding, and records final session cleanup state as `abandoned` or `cleanup-queued`.
- Authoring UI source now aborts in-flight Listening uploads and queues trusted cleanup by `uploadSessionId` plus `assetId` only on replacement, section remove, discard, and upload abort. Cleanup calls retry before surfacing diagnostic warnings; the browser still never receives raw R2 delete authority.
- Proof passed focused app tests, focused Worker tests including R2-delete-failure retry coverage, production app build, Worker dry-run, production Worker deploy, trusted cleanup route execution for the nine explicitly approved `temp/listening/glMHCrzMnyS6AqFcb9I0nlOqQ6X2/...` candidates, and post-cleanup remote verification. R2 HEAD verification returned 404 for all nine candidate keys, RTDB upload sessions are `abandoned` with `builder-cancel`, `/media_assets/{assetId}` is null for all nine assets, durable roots had zero asset/key hits, and permanent prefixes touched count was zero. Historical packet note now retired: at the moment of the nine-object cleanup, R2 lifecycle had not yet been provisioned; the follow-up permanent setup below supersedes that state.
- Follow-up permanent setup was deployed on 2026-07-02 with Worker version `dca3e056-142f-4cb4-9194-3117675f8889` at 100%, cron trigger `0 * * * *`, `LISTENING_UPLOAD_SESSION_SWEEP_ENABLED=true`, and future-only cutoff `LISTENING_UPLOAD_SESSION_SWEEP_NOT_BEFORE_MS=1782976636347`. The scheduled sweep uses the same trusted cancel service for expired future `active` or `cleanup-queued` Listening upload sessions, writes aggregate `media_asset_sweeps/{sweepId}` and `media_asset_metrics/{metricEventId}` records without raw temp keys, and does not perform historical orphan deletion or durable pending-delete cleanup.
- RTDB rules for `temp-a1437-default-rtdb` were deployed with the expanded lifecycle metric operation allowlist. R2 lifecycle readback for bucket `kahoot-media` now shows the default multipart abort rule plus enabled rule `expire-temp-prefix-after-one-day` for prefix `temp/`, action `Expire objects after 1 days`.
...
| Scheduled/reconciliation work | Cloudflare Worker scheduled/cron-style owner for future Listening temp-session cleanup; approved local planner or later trusted Worker packets for broader reconciliation and durable cleanup. |
...
6. Superseding current status, 2026-07-02: abandoned Listening upload cleanup now has deployed Worker route proof, deployed hourly future-only scheduled sweep setup, deployed RTDB metric-operation rules, and provisioned R2 `temp/` one-day lifecycle fallback. Durable cleanup execution, historical orphan deletion, first observed scheduled sweep run, wider rollout, staging, commit, and push remain separately gated.
