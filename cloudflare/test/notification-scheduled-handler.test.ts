...
describe('composed notification Worker scheduled handler', () => {
  it('dispatches the selected bounded family through waitUntil', async () => {
    ...
    worker.scheduled({ cron: '*/2 * * * *', scheduledTime: 0 }, {}, { waitUntil: (promise: Promise<unknown>) => { pending = promise; } });
    await pending;
  ...
  it('gives the bulk queue its own minute trigger', async () => {
    let pending: Promise<unknown> | undefined;
    worker.scheduled({ cron: '* * * * *', scheduledTime: 0 }, {}, { waitUntil: (promise: Promise<unknown>) => { pending = promise; } });
    await pending;
