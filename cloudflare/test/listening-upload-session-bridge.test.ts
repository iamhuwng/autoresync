...
describe('PRD-0056A Worker bridge grant', () => {
  ...
  it('blocks scheduled cleanup while restore is active before writing sweep state', async () => {
    const repository = createMemoryRepository();
    ...
    await expect(handler.scheduled({
      env: { LISTENING_UPLOAD_SESSION_SWEEP_ENABLED: 'true' },
      cron: '0 * * * *',
    })).resolves.toEqual({
      schemaVersion: 1,
    ...
    expect(repository.sweepRecords).toHaveLength(0);
    expect(repository.metricRecords).toHaveLength(0);
  });

  it('caches durable reference roots for one repository sweep lifetime', async () => {
    ...
    const input = {
      ownerId: 'teacher-1',
      assetIds: ['asset-55555555555555555555555555555555'],
      tempKeys: ['temp/listening/teacher-1/session/asset.mp3'],
    } as const;

    await repository.findDurableAssetReferences(input);
    await repository.findDurableAssetReferences(input);
    ...
    for (const root of [
      ...
      expect(fetchImpl.mock.calls.filter(([request]) => String(request).includes(`/${root}.json`))).toHaveLength(1);
    }
  });
});
