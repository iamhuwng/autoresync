...
describe('Backblaze B2 private Source provider adapter', () => {
  ...
  it('verifies exact immutable metadata, reads only bounded ranges, and rejects provider drift', async () => {
    ...
    const provider = createProvider(withValidatedAuthority(async (input, init) => {
      ...
      if (init?.method === 'HEAD') return new Response(null, { headers: {
        ...
        'x-amz-meta-book-source-byte-size': '6', 'x-amz-meta-book-source-sha256': checksum,
        etag: '"not-the-sha256-checksum"', 'x-bz-file-id': '4_zabc', 'x-amz-version-id': '4_zabc',
      } });
    ...
    expect(new URL(calls[0]!.url).searchParams.get('versionId')).toBe(identity.providerFileVersionId);
    expect(new URL(calls[1]!.url).searchParams.get('versionId')).toBe(identity.providerFileVersionId);
    expect(new URL(calls[0]!.url).search.slice(1).startsWith('X-Amz-Algorithm=')).toBe(true);
  ...
  it('preserves distinct provider file and version identifiers', async () => {
    const distinct = {
      ...identity,
    ...
    const provider = createProvider(withValidatedAuthority(async (input, init) => {
      ...
      if (init?.method === 'HEAD') {
        return new Response(null, { headers: {
          ...
          'x-amz-meta-book-source-byte-size': '6',
          'x-amz-meta-book-source-sha256': checksum,
          'x-bz-file-id': distinct.providerFileId,
          'x-amz-version-id': distinct.providerFileVersionId,
        } });
      }
      return new Response(new Uint8Array([2, 3]), {
        status: 206,
        headers: { 'content-range': 'bytes 2-3/6' },
      });
    }));

    await expect(provider.verifyCompletedObject({ expected: distinct })).resolves.toMatchObject({
      identity: distinct,
    });
    await expect(provider.readBounded({ identity: distinct, range: { offset: 2, length: 2 } }))
      .resolves.toEqual({ bytes: new Uint8Array([2, 3]), totalByteSize: 6, offset: 2 });
    expect(new URL(calls[0]!.url).searchParams.get('versionId')).toBe(distinct.providerFileVersionId);
    expect(new URL(calls[1]!.url).searchParams.get('versionId')).toBe(distinct.providerFileVersionId);
  });
  ...
  it('reads one bounded B2 total page and leaves accumulation to caller', async () => {
    ...
    const provider = createProvider(withValidatedAuthority(async (input, init) => {
      ...
      if (url.includes('list_file_versions')) {
        ...
        ], nextFileName: 'next.pdf', nextFileId: '4_next' }) : Response.json({ files: [
          { action: 'upload', fileId: '4_next', fileName: 'next.pdf', contentLength: 4 },
        ] });
      }
    ...
    expect(JSON.parse(calls.find((call) => call.url.includes('list_file_versions'))!.body ?? '{}')).toMatchObject({ maxFileCount: 3 });
  ...
  it('fails closed for B2 account-total continuation self-loops or invalid page sizes', async () => {
    const provider = createProvider(withValidatedAuthority(async (input) => {
      if (String(input).includes('list_file_versions')) return Response.json({
        files: [], nextFileName: 'same.pdf', nextFileId: '4_same',
      });
  ...
  it('rejects broad, wrong-bucket, prefix-inconsistent, or endpoint-drifted operation authority', async () => {
    ...
    await expect(providerFor({ buckets: bucket, capabilities: ['writeFiles'], namePrefix: 'other-prefix/' })
      .authorizeUpload(upload)).rejects.toMatchObject({ code: 'unauthorized' });
    await expect(providerFor({ buckets: bucket, capabilities: ['writeFiles'], namePrefix: null }, 'https://s3.us-east-005.backblazeb2.com')
      .authorizeUpload(upload)).rejects.toMatchObject({ code: 'provider_drift' });
    await expect(providerFor(
      { buckets: bucket, capabilities: ['writeFiles'], namePrefix: null },
