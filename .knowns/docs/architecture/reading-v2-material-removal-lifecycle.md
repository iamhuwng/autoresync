...
description: Reading V2 PRD-0054 removal contract for master full-test soft removal, optional linked-passage archive, stale Material Catalog cleanup, audit events, and frozen work safety.
...
Master-only removal sets master composition/metadata state to `removed`, removes active Material Catalog index rows, removes legacy `/tests/{masterMaterialId}`, writes `reading_master_removed` audit, and leaves linked Reading Passages active.
...
Material Catalog cleanup must be idempotent for stale/missing active index rows. Owner cleanup may use canonical `reading_v2/material_metadata/{materialId}/ownerId === auth.uid` as fallback proof.
